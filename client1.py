# 导入所需的模块
import socket
import tkinter as tk #利用python中的tk模块，进行图形化设计

# 设置常量
HEADER_LENGTH = 10 #设计最大输入长度，其次保证接受数据时有报文头部方便构建整条消息
IP = "127.0.0.1" #本地环回测试
PORT = 1234
...
# 创建消息显示框
messages_frame = tk.Frame(root) #frame方法用于构建矩形框架
...
# 创建消息输入框
entry_field = tk.Entry(root, textvariable=my_msg)#entry方法用于创建一个文本框
entry_field.pack()
...
def receive_message():
    while True:
        try: #捕获异常
            ...
            msg_list.insert(tk.END, f'{username} > {message}')#实现查看自己与对方的聊天记录
